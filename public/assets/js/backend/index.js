define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    ...
    function init() {
        ws.onopen = function () {
            // 连接或重连后刷新会话标识
            ws.send(JSON.stringify({type: 'authorize',admin_id: Config.user.id}));
            $('.addtabsit > img').removeClass('socket-offline').addClass('socket-online').prop('title','WebSocket在线');
            heartCheck.start();
        };
        ws.onclose = function () {
            ...
            $('.addtabsit > img').removeClass('socket-online').addClass('socket-offline').prop('title','WebSocket离线');
            reconnect(wsUrl);
        };
        ws.onerror = function() {
            ...
            switch(data.type) {
                ...
                case 'clues':
                    ...
                    var index = Layer.open({
                        ...
                        area: ['300px','auto'],
                        btn: ['查看','全部关闭'],
                        content: '<div style="padding:20px">您收到新的客户线索，姓名：'+data.user.name+'，点击查看</div>',
    ...
    var Controller = {
        index: function () {
            $(document).on('click', '.on-line', function() {
                var status = $(this).data('status');
                Fast.api.ajax({
                    url: 'general/profile/online',
                    data: {status}
                }, function(data) {
                    var html = '';
                    if (status == 'online') {
                        html = '<i class="fa fa-circle text-success"></i> <a href="javascript:;" class="on-line" data-status="offline">在线</a>'
                    } else {
                        html = '<i class="fa fa-circle text-warning"></i> <a href="javascript:;" class="on-line" data-status="online">离线</a>'
                    }
                    $('.info').children('div').html(html)
                }, function(err) {
                    return false;
                })
            })
